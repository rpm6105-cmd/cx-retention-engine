          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              ...
              <div className="mt-1 flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
                ...
                <Badge tone={healthScore < 40 ? "danger" : healthScore < 70 ? "warning" : "success"}>
                  Health · {healthScore}
                </Badge>
                <Badge tone={riskLevel === "High" ? "danger" : riskLevel === "Medium" ? "warning" : "neutral"}>
                  {risk}
                </Badge>
              </div>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
          <main className="space-y-6 lg:col-span-8">
              ...
              <CardBody>
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="text-right">
                    <div className="text-xs font-semibold uppercase tracking-wide text-gray-500">Overall</div>
                    <div className="mt-1 text-2xl font-semibold tabular-nums text-gray-900">{healthScore}</div>
                  </div>
                </div>
                <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-3">
                  <MetricBar label="Usage" value={breakdown.usage} tone="neutral" />
                  <MetricBar label="Support" value={breakdown.support} tone={breakdown.support >= 70 ? "danger" : "warning"} />
                  <MetricBar label="Engagement" value={breakdown.engagement} tone={breakdown.engagement >= 70 ? "success" : "warning"} />
                </div>
              </CardBody>
            </Card>
            ...
            <Card>
              <CardBody>
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-gray-600">Mock usage trend (last 12 weeks).</p>
                </div>
            ...
            <Card>
              <CardBody>
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-gray-600">Drivers behind churn risk and required follow-ups.</p>
                </div>
                <div className="mt-4 space-y-3">
                  {riskIndicators.map((r) => (
                    <div key={r.title} className="flex items-start justify-between gap-3 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
            ...
            <Card className="overflow-hidden">
              <CardHeader className="border-b border-gray-200">
                ...
                <p className="mt-0.5 text-xs text-gray-600">Work items tied to this account.</p>
              </CardHeader>
              <div className="divide-y divide-gray-100">
                {customer.tasks.map((t) => (
                  <div key={t.id} className="flex items-center justify-between gap-3 px-4 py-4 transition-colors hover:bg-gray-50/70 sm:px-5">
                    <div className="min-w-0">
                      <div className="truncate text-sm font-semibold text-gray-900">{t.title}</div>
                      <div className="mt-0.5 text-xs text-gray-600">Due {t.due}</div>
                    </div>
          ...
          <aside className="lg:col-span-4">
            <div className="sticky top-4 space-y-4">
                ...
                <CardBody>
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-0.5 text-xs text-gray-600">Quick actions for CS workflows.</p>
                    </div>
                  ...
                  {isStarter ? (
                    <div className="mt-4 rounded-2xl border border-indigo-100 bg-indigo-50 p-4 text-center">
                      <div className="grid h-10 w-10 mx-auto place-items-center rounded-xl bg-indigo-600 text-white shadow-sm shadow-indigo-600/20">
                        <svg viewBox="0 0 20 20" fill="currentColor" className="h-5 w-5" aria-hidden="true">
                      ...
                      <p className="mt-1 text-xs text-indigo-700">
                        Upgrade to Pro or Business to unlock account summaries, next action suggestions, and email drafts.
                      </p>
                      <Link
                        href="/profile"
                        className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm shadow-indigo-600/20 transition hover:bg-indigo-700 focus:outline-none focus:ring-4 focus:ring-indigo-600/20"
                      >
                        Upgrade plan →
                      </Link>
                    </div>
                    ...
                    <>
                      <div className="mt-4 space-y-2">
                        <CopilotButton
                          ...
                          onClick={() => {
                            const issues = keyIssues.length > 0 ? keyIssues.join(", ") : "No major issues detected";
                            const inactivity = Number.isFinite(daysSinceActivity) && daysSinceActivity !== Number.POSITIVE_INFINITY ? `${daysSinceActivity}d` : "—";
                            setCopilotOutput([
                      ...
                      <div className="mt-4 rounded-2xl border border-gray-200 bg-gray-50 p-3">
                        ...
                        <pre className="mt-2 whitespace-pre-wrap text-sm text-gray-700">{copilotOutput}</pre>
                      </div>
                    </>
                  )}
                </CardBody>
              ...
              <Card>
                <CardBody>
                  <h3 className="text-sm font-semibold text-gray-900">Account snapshot</h3>
...
function MetricBar({ label, value, tone }: { label: string; value: number; tone: "neutral" | "warning" | "danger" | "success" }) {
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
      ...
      <div className={`mt-3 h-2 w-full rounded-full ${bars[tone].bg}`}>
        <div className={`h-2 rounded-full ${bars[tone].fill}`} style={{ width: `${clamp(value, 0, 100)}%` }} aria-hidden="true" />
      </div>
  ...
  return (
    <div className="w-full">
      <div className="flex items-center justify-between">
        ...
        <div className="text-xs text-gray-600">Avg <span className="font-semibold tabular-nums text-gray-900">{Math.round(safe.reduce((a, b) => a + b, 0) / safe.length)}</span></div>
      </div>
      <svg className="mt-3 h-32 w-full" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" role="img" aria-label="Usage trend">
        <defs>
        ...
        <path d={`M ${points.replaceAll(" ", " L ")} L ${w - pad},${h - pad} L ${pad},${h - pad} Z`} fill="url(#usageFill)" />
        <polyline points={points} fill="none" stroke="rgb(17 24 39)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    </div>
  );
}
...
function CopilotButton({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button onClick={onClick} className="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-left text-sm font-semibold text-gray-900 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-4 focus:ring-indigo-600/15">
      {label}
    </button>
  );
}
...
function indicatorsForRisk(riskLevel: "High" | "Medium" | "Low") {
  if (riskLevel === "High") {
    return [
      { title: "Low usage", detail: "Product activity is significantly below baseline; prioritize adoption recovery.", severity: "high" as const },
      { title: "High support tickets", detail: "Ticket volume indicates friction; align on root cause and resolution plan.", severity: "high" as const },
      { title: "Recent inactivity", detail: "Stakeholder engagement has dropped; schedule an urgent check-in.", severity: "medium" as const },
    ];
  }
  if (riskLevel === "Medium") {
    return [
      { title: "Adoption dip", detail: "Usage is trending down; identify the workflow with the biggest drop-off.", severity: "medium" as const },
      { title: "Support load", detail: "Moderate ticket volume; confirm SLA and prevent repeat issues.", severity: "medium" as const },
      { title: "Engagement gaps", detail: "Stakeholder touches are inconsistent; propose a lightweight success plan.", severity: "low" as const },
    ];
  }
  return [
    { title: "Strong usage", detail: "Usage levels are healthy and consistent across recent weeks.", severity: "low" as const },
    { title: "Low support friction", detail: "Ticket volume is low; keep monitoring and share best practices.", severity: "low" as const },
    { title: "Positive engagement", detail: "Engagement signals look stable; maintain cadence with periodic check-ins.", severity: "low" as const },
  ];
}
