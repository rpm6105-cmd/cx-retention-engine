  ...
  const sidebarContent = (
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center gap-3 px-2">
        ...
        {/* Mobile close */}
        <button className="lg:hidden grid h-8 w-8 shrink-0 place-items-center rounded-lg text-white/60 hover:bg-white/10 hover:text-white" onClick={() => setMobileOpen(false)} aria-label="Close menu">
      ...
      <div className={`mt-4 rounded-2xl border border-white/10 bg-white/5 p-3 ${collapsed ? "flex justify-center" : ""}`}>
        {collapsed ? (
          <button onClick={handleLogout} className="grid h-8 w-8 place-items-center rounded-lg text-white/50 transition hover:bg-white/10 hover:text-white" title="Sign out" aria-label="Sign out">
    ...
    <>
      ...
      <div className="fixed top-0 left-0 right-0 z-40 flex items-center justify-between border-b border-white/10 bg-slate-950 px-4 py-3 lg:hidden">
        <div className="flex items-center gap-2">
          <div className="grid h-8 w-8 place-items-center rounded-lg bg-gradient-to-br from-indigo-500 to-violet-600 text-white"><span className="text-xs font-extrabold">CX</span></div>
