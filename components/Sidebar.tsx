...
export default function Sidebar() {
        ...
        {items.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              onClick={() => setMobileOpen(false)}
              className={`group flex items-center gap-3 rounded-xl px-3 py-2 text-sm font-semibold transition ${active ? "bg-white/10 text-white ring-1 ring-white/10" : "text-white/80 hover:bg-white/5 hover:text-white"}`}
            >
              <span className={`grid h-8 w-8 place-items-center rounded-lg transition ${active ? "bg-indigo-500/20 text-indigo-200" : "text-white/70 group-hover:text-white"}`} aria-hidden="true">
                <Icon />
              </span>
  ...
  return (
    <>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 bg-slate-950/60 backdrop-blur-sm lg:hidden" onClick={() => setMobileOpen(false)} aria-hidden="true" />
      )}
